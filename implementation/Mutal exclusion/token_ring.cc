...
int main(int argc, char* argv[])
{
    ...
    // Philosophers 1 and 2 are hungry
    ...
    if (id == 0)
    {
        ...
        send_message(id+1, OK);
    }
    
    while (1)
    {
        int rv = poll(ufds, kTotalProcesses, 3500);
		if (rv == -1) // Error
			exit(1);
